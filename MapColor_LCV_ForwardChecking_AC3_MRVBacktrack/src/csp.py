...
class CSP(search.Problem):
    ...
    def numAssignments(self):
        """Self reports number of assignments"""
        return self.nassigns
    
    def replaceNeighbors(self, neighbors):
        """Allows for restoration of neighbors"""
        self.neighbors = neighbors
        
    def replaceVariables(self, variables):
        """Allows for restoration of variables"""
        self.variables = variables
...
def cutset_conditioning(csp):
    """Function to determine the cutset and the resultant tree and orchestrates the following
    solutions to both the cutset and the tree colorings"""
    
    originalcsp = copy.deepcopy(csp)
    neighbors = csp.neighbors
    nlist = list(neighbors.items())
    removals = []
    cutset_ratios = []
    cutset = []

    #Initial removal step
    while True:
        listsize = nlist.__len__()
        for country, neighbor_list in nlist:
            deg_0_1 = neighbor_list.__len__() == 0 or neighbor_list.__len__() == 1
            if deg_0_1:
                removals.append(country)
                nlist = [i for i in nlist if i[0] != country]
                
        #remove from neighbors what just went into removals step
        for country, neighbor_list in nlist:
            for remove in removals:
                if remove in neighbor_list:
                    neighbor_list.remove(remove)
        
        #size did not change, no need to repeatedly check anymore
        if nlist.__len__() == listsize:
            break
    
    #Cutset Algorithm main loop
    while nlist:
        node_getting_cut = ''
        cutset_ratios = []
                
        #cutset step, generate degree ratios
        for country, neighbor_list in nlist:            
            if neighbor_list.__len__() != 0:
                cutset_ratio = 1/neighbor_list.__len__()
                cutset_ratios.append((country, cutset_ratio))
            
        #check ratios for cut if necessary
        most_edges = 1.0
        for country, cutset_ratio in cutset_ratios:
            if cutset_ratio < most_edges:
                most_edges = cutset_ratio
                node_getting_cut = country
        
        #cut the node and remove from neighbors
        if node_getting_cut != '':
            cutset.append(node_getting_cut)
            nlist = [i for i in nlist if i[0] != node_getting_cut]

        #remove from neighbors what just went into cutset
        for country, neighbor_list in nlist:
            for cut in cutset:
                if cut in neighbor_list:
                    neighbor_list.remove(cut)
                    
        #removal step
        while True:
            listsize = nlist.__len__()
            #determine nodes with degree 0 or 1 and remove if found
            for country, neighbor_list in nlist:
                deg_0_1 = neighbor_list.__len__() == 0 or neighbor_list.__len__() == 1
                if deg_0_1:
                    removals.append(country)
    ...
    solve_tree(originalcsp, cutset, solve_cutset(originalcsp, cutset, removals))
    

def solve_cutset(csp, cutset, removals):
    """Function which solves the cutset portion after performing cutset conditioning"""
    
    cutset_csp = copy.deepcopy(csp)
    original_neighbors = cutset_csp.neighbors
    original_variables = cutset_csp.variables
    
    for r in removals:       
        #remove the cuts from the variable list
        original_variables.remove(r)
        
        #remove the key value pairs of the cuts
        del original_neighbors[r]
        
    #remove from neighbors the cuts
    for _,v in original_neighbors.items():
        for r in removals:
            if r in v:
                v.remove(r)
                
    #the tree will need a minimum of two colors
    for color in cutset_csp.domains.value:
        i = 1
        if i < 2:
            cutset_csp.domains.value.remove(color)
            i = i + 1
            
    #solve the cutset using backtracking search
    backtracking_search(cutset_csp, order_domain_values=lcv)
    
    #Restore the variables and neighbors after cutset manipulation
    cutset_csp.replaceVariables(csp.variables)
    cutset_csp.replaceNeighbors(csp.neighbors)
    
    return cutset_csp

def solve_tree(csp, cutset, cutset_csp):
    """Function which solves the tree portion after performing cutset conditioning"""
    
    original_neighbors = csp.neighbors
    original_variables = csp.variables
    
    for c in cutset:       
        #remove the cuts from the variable list
        original_variables.remove(c)
        
        #remove the key value pairs of the cuts
        del original_neighbors[c]
        
    #remove from neighbors the cuts
    for k,v in original_neighbors.items():
        for c in cutset:
            if c in v:
                v.remove(c)
    
    
    #remove colors used by the cutset solution      
    for k,v in cutset_csp.curr_domains.items():
        for color in v:
            if color in csp.domains.value:
                csp.domains.value.remove(color)
  
    tree = csp
    tsp_solution = tree_csp_solver(tree)
    
    #combine the two solutions
    for k,v in cutset_csp.curr_domains.items():
        tsp_solution.update( {k : v.pop()} )
    
    print("Tree Solution Combined with Cutset Solution")
    tree.display( tsp_solution )   
    print(" ")
...
def parse_neighbors(neighbors, variables=None):
    ...
    #had to include this to remove duplicate neighbors, otherwise cutset algorithm cannot compute correctly
    #I am the author of the piazza post
...
# ______________________________________________________________________________
# The Ship Puzzle
# rules can be found at: http://brainden.com/einsteins-riddles.htm
...
# Solution should be a variation of (rows should hold these values):
# 
# French    | Five  | Tea    | Blue  | Genoa
# Greek     | Six   | Coffee | Red   | Hamburg
# Brazilian | Eight | Cocoa  | Black | Manila
# English   | Nine  | Rice   | White | Marseille
# Spanish   | Seven | Corn   | Green | PortSaid


def Ships():
    """Return an instance of the Ships Puzzle."""
    
    Ships = 'French Greek Brazilian English Spanish'.split()
    Times = 'Five Six Seven Eight Nine'.split()
    Payloads = 'Coffee Cocoa Tea Rice Corn'.split()
    Colors = 'Blue Red Black White Green'.split()
    Ports = 'Genoa Hamburg Manila Marseille PortSaid'.split()
    variables = Ships + Times + Payloads + Colors + Ports
    ...
    neighbors = parse_neighbors("""
            ...
            Spanish: Seven;
            Red: Hamburg;
            Hamburg: Six""", variables)
    
    for types in [Ships, Times, Payloads, Colors, Ports]:
        for A in types:
            for B in types:
                if A != B:
                    if B not in neighbors[A]:
    ...
    #implemented constraints based on problem definition
    def ships_constraint(A, a, B, b, recurse=0):
        same = (a == b)
        next_to = abs(a - b) == 1
        
        #values to be placed in same row
        if A == 'Greek' and B == 'Six':
            return same
        if A == 'Greek' and B == 'Coffee':
            return same
        if A == 'English' and B == 'Nine':
            return same
        if A == 'French' and B == 'Blue':
            return same
        if A == 'Brazilian' and B == 'Manila':
            return same
        if A == 'Genoa' and B == 'Five':
            return same
        if A == 'Spanish' and B == 'Seven':
            return same
        if A == 'Red' and B == 'Hamburg':
            return same
        if A == 'Black' and B == 'Eight':
            return same
        if A == 'Hamburg' and B == 'Six':
            return same
        
        #values in adjacent rows     
        if A == 'Rice' and B == 'Green':
            return next_to
        if A == 'Seven' and B == 'White':
            return next_to
        if A == 'Corn' and B == 'Rice':
            return next_to

        #B is on the left of A
        if A == 'Coffee' and B == 'French':
            return a - 1 == b
        if A == 'Marseille' and B == 'Cocoa':
            return a - 1 == b
        if A == 'Marseille' and B == 'Spanish':
            return a - 1 == b
        if A == 'Green' and B == 'White':
            return a - 1 == b
        if A == 'Spanish' and B == 'English':
            return a - 1 == b          
        
        if recurse == 0:
            return ships_constraint(B, b, A, a, 1)
        if ((A in Colors and B in Colors) or
                (A in Ships and B in Ships) or
                (A in Times and B in Times) or
                (A in Payloads and B in Payloads) or
                (A in Ports and B in Ports)):
            return not same
        raise Exception('error')
    return CSP(variables, domains, neighbors, ships_constraint)
